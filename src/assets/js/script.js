...
// const previewNumbers = document.querySelectorAll(".preview-numbers");
// function handleStepperButton(btn) {
//     let buttonId = btn.getAttribute("id");
//     let minValue = previewInput.getAttribute("min");
//     let maxValue = previewInput.getAttribute("max");
//     let stepValue = previewInput.getAttribute("step");
//     let currentValue = previewInput.getAttribute("value");
//     let stepIncrement =
//         buttonId == "increment" ? stepValue * 1 : stepValue * -1;
//     let newValue = parseInt(currentValue) + stepIncrement;
//     if (newValue >= minValue && newValue <= maxValue) {
//         previewInput.setAttribute("value", newValue);
//     }
// }
// previewNumbers.forEach((number) => {
//     number.addEventListener("click", function () {
//         handleStepperButton(this);
//     });
// });
...
magnifierImg.addEventListener("mousemove", function (event) {
    let clientX = event.clientX - magnifierImg.offsetLeft;
    let clientY = event.clientY - magnifierImg.offsetTop;
    let mWidth = magnifierImg.offsetWidth;
    let mHeight = magnifierImg.offsetHeight;
    clientX = (clientX / mWidth) * 100;
    clientY = (clientY / mHeight) * 100;
    magnifierImg.style.transform = `translate(-${clientX}%, -${clientY}%) scale(2)`;
});
magnifierImg.addEventListener("mouseleave", function () {
    magnifierImg.style.transform = "scale(1)";
});
